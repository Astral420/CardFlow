...
2. auth.login() applies it per-client and returns 429 once exhausted,
   and still uses a constant-time comparison for the passcode.
"""
...
def test_login_rejects_wrong_passcode_without_tripping_rate_limit_early():
    db = _make_session()
    ...
    with pytest.raises(HTTPException) as exc_info:
        login(
            LoginRequest(name="alex", passcode="wrong"),
...
def test_login_blocks_after_too_many_attempts_from_same_ip():
    ...
    with pytest.raises(HTTPException) as exc_info:
        login(
            LoginRequest(name="alex", passcode=settings.app_passcode),
            _fake_request("9.9.9.9"),
            db=db,
        )
